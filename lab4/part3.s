...
ISREND:			LDR      R0, =0xFF20005C 	// Reset Edgebit
				MOV      R3, #15
				STR      R3, [R0]
                B        EXIT_IRQ				
...
KEY0PRESSED:    LDR      R0, =RUN        	// Invert RUN Bit
				LDR      R1, [R0]
				CMP      R1, #0
				MOVEQ    R1, #1
				MOVNE    R1, #0
...
KEY1PRESSED:	LDR      R0, =0xfffec600 // Timer
				LDR      R2, [R0]		 // Speed x2
				LSR      R2, #1
				MOV      R1, #0b101       // bit pattern (I = 1, A = 0, E = 1)
				STR      R1, [R0, #0x8]
				STR      R2, [R0]		  // Load Value
				MOV      R1, #0b111       // bit pattern (I = 1, A = 1, E = 1)
				STR      R1, [R0, #0x8]
				BEQ      ISREND


KEY2PRESSED:	LDR      R0, =0xfffec600 // Timer
				LDR      R2, [R0]		 // Speed /2
				LSL      R2, #1
				MOV      R1, #0b101       // bit pattern (I = 1, A = 0, E = 1)
				STR      R1, [R0, #0x8]
